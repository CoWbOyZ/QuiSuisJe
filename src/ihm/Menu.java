...
public class Menu {
	...
	/**
	 * 
	 * @param lignes est une liste de chaine de caractères. Chaque chaine contient un choix possible du menu (ex: nouvelle partie, option,...)
	 * @return choix 
	 */
	public static int getChoix(ArrayList<String> lignes){
		...
		try{
			...
			//Affiche les différentes options du menu généré à l'aide de la liste lignes
			for (int i = 0; i < nbChoix; i++) {
				System.out.println(i+" : "+lignes.get(i));
			}

			//Ajoute/Affiche l'option quitter
			...
			do{
				...
				choix = sc.nextInt(); 
			}while (choix<0 || choix>nbChoix);//tant que le choix saisi ne correspond pas à une option
		...
		//  Si la saisie correspond à la derniere ligne
		if (choix==nbChoix) {
			choix = -1;
		}
